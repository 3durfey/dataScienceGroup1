...
class ScoreDistribution:
    ...
    def _get_rating_distribution(self, rate):
        """
        ...
        while sum(distribution) < 10:
            if flag == 1:
                if distribution[0] >= 2 or (distribution[0] == rate):
                    distribution.insert(1, 1)
                elif distribution[0] == 1:
                    distribution[0] += 1
            elif flag == -1:
                if distribution[-1] == rate or (distribution[-1] >= 2):
    ...
    def _get_final_rate_distribution(self):
        ...
        """
        distribution = self._get_rating_distribution(self.rate)
        final_distribution = np.append(np.zeros(9), distribution)
        final_distribution = np.append(final_distribution, np.zeros(9))
        return final_distribution[len(final_distribution) // 2 - self.primary: len(final_distribution) // 2 - self.primary + self.num_classes]
